...
def printInv(i):
    ...
    for index, item in enumerate(all_tuples[i]):
        if index == 0:      #if it is the first item (character's name)
            print(item)
            """This customizes length of the line"""
...
char = input("Which character inventory would you like to see?: ")
char = char.capitalize()                #Capitalizes the first letter automatically
while char not in characters:
    #This prevents user from typeing character that is not in the list
    print("That is not a valid character. These are the characters in the game: ")
    print(characters)
    char = input("Which character inventory would you like to see?: ")
    char = char.capitalize() 
ind = characters.index(char)            #finds the index of the character in characters list
print("")
printInv(ind)                           #prints the inventory of that character using the function
