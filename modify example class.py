...
def pick_char():
    ...
    print("")
    return ind                              #return the value of ind when I run function
...
while (variable != 4):
    ...
    print("4: Quit")
    print("")
    variable = int(input("What command do you want to enter?: "))    #ask for choice


                                            #conditional to print based on the choice
    if variable == 1:
        index = pick_char()
        printInv(index)                     #prints the inventory of that character using the function
    if variable == 2:
        index = pick_char()
        added = input("What would you like to add to " + characters[index] + "'s inventory?: ")
        added = added.lower()               #make sure user input is lowercase
        all_tuples[index].append(added)     #adds item to character's inventory
        print("")                           #prints a blank line
        printInv(index)                     #prints the character's new inventory
    if variable == 3:
        new = input("What new character do you want to add?: ")
        new = new.capitalize()
        characters.append(new)
        new_char = [new,]                   #creating a new list with only character name
        num_items = int(input("How many item's in the inventory?: "))
        for i in range(num_items):
            item = input("Enter an item: ")
            new_char.append(item)
        all_tuples.append(new_char)
        #index = len(characters)-1           #index of last item in list
        print("")
        printInv(-1)
