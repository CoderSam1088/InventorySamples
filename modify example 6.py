...
def add_char():
    to_add = input("What is the name of your character?: ")
    to_add = to_add.capitalize()
    characters.append(to_add)
    all_tuples.append([to_add])
    all_holding.append([to_add])
    index = characters.index(to_add)
    num_items = int(input("How many items are in the character's inventory?: "))
    for i in range(num_items):
        item = input("Item: ")
        item = item.lower()
        all_tuples[index].append(item)
    print("")
    printInv(index)
...
while (choice != 5):
    ...
    if(choice == 1):
        ...
        all_tuples[index].remove(item_remove)
        printInv(index)
    print("")
